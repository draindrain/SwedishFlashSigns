    ...
    <div className="bg-white rounded-xl border border-slate-200 p-4">
      <div className="flex gap-4 items-start">
        ...
        <div className="flex flex-col items-center gap-1">
          ...
          <p className="text-xs text-center text-slate-600 max-w-[80px] leading-tight">
            {wrongSign?.name ?? selectedId}
          </p>
        </div>

        <div className="flex flex-col items-center justify-center pt-5 text-slate-400 text-xl font-bold">
          →
        </div>
        ...
        <div className="flex flex-col items-center gap-1">
          ...
          <p className="text-xs text-center text-slate-800 font-medium max-w-[80px] leading-tight">
            {correctSign.name}
          </p>
        </div>
      </div>
      <p className="mt-2 text-xs text-slate-400">{correctSign.categoryName} · {correctSign.id}</p>
    </div>
  )
}
...
export default function ResultsScreen({ answered, onRetry, onHome }: Props) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-50 to-white flex flex-col items-center pt-16 pb-16 px-4">
      <div className="max-w-lg w-full space-y-6">

        {/* Score card */}
        <div className="bg-white rounded-2xl border border-slate-100 shadow-md p-8 text-center">
          ...
          <div className="text-5xl font-black text-slate-900">{correct} / {total}</div>
          <div className="text-lg text-slate-600 mt-1">rätt svar</div>
          {wrong.length > 0 && (
        ...
        <div className="flex gap-3">
          <button
            onClick={onRetry}
            className="flex-1 py-3 bg-slate-800 hover:bg-slate-900 text-white font-semibold rounded-xl transition-all shadow-sm hover:shadow-md"
          >
            Försök igen
          </button>
          <button
            onClick={onHome}
            className="flex-1 py-3 bg-white hover:bg-slate-50 text-slate-700 font-semibold rounded-xl border border-slate-200 transition-colors"
          >
            Ny session
          </button>
        </div>
        ...
        {wrong.length > 0 && (
          <div>
            <button
              ...
              className="w-full py-3 text-sm font-semibold text-amber-500 hover:text-amber-600 flex items-center justify-center gap-2"
            >
